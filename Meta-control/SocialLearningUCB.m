function results = SocialLearningUCB(settings, env)
...
for t=1:settings.T
    ...
    for i=1:numOfAgents
        ...
        [mv, strategySelection] = max(agents{i}.socialReward + settings.confLevel.*(sqrt(log(t)./agents{i}.N)));    
        
        if(t==1)
            strategySelection = 3;
        end
        agents{i}.N(strategySelection) = agents{i}.N(strategySelection) + 1;
