function results = SocialLearningGA(settings, env)

genotype1 = settings.genotype(1:2);
genotype2 = settings.genotype(3:end);
...
for t=1:settings.T
    ...
    if(t>1)
        ...
        if(~isnan(rewMeanBest) && ~isnan(rewStdBest) &&...
            ...
            ODPU = uncertaintyFormula(rewMeanBest,rewStdBest,k1,rewMeanRest,rewStdRest,k2);end
        if(ODPU >= genotype1(1))
            uncertain = 1;
        elseif(isnan(ODPU))
%             uncertain = NaN;
        ...
        if(t>2)
            if(abs(rewMeans(mrInx,t-2)-rewMeans(mrInx,t-1)) > genotype1(2))
                converged = 0;
                kObs = 0;
                settings.pIL = 1;
                environmentChange = 1;
    ...
    %Strategy selection based on the Genotype of the evolved strategy
    %converged uncertain kObs>kThreshold=>0
    ...
    sinx = genotype2(stateInx);
    if(sinx~=0), strategyInx = sinx; end
    if(t==1), strategyInx = 3; end
