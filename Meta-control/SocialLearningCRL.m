function results = SocialLearningCRL(settings, env)
...
for t=1:settings.T
    ...
    if(t-settings.tau > 0)
        ...
        varCoef(t) = var(v(v~=0));
        %         varCoef(t) = var(v(v~=0)./settings.nArm);%sum(h(:,2)>1)/settings.nArm;
...
%             uncertain = NaN;
        ...
        if(t>2)
            if(rewMeans(mrInx,t-2)./rewMeans(mrInx,t-1) > 1.15)
                converged = 0;
                kObs = 0;
                settings.pIL = 1;
                strategyInx = 3;
        ...
        if(~uncertain && converged && kObs > 1)
            strategyInx = 1;
            settings.pIL = 0;
        elseif(~uncertain && ~converged && kObs > 1)
            strategyInx = 1;
            settings.pIL = 0;
        end
    ...
    epsilon = epsilon*settings.epsilonDecay;
