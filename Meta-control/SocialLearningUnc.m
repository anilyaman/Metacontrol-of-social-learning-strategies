function results = SocialLearningUnc(settings, env)
...
for t=1:settings.T
    ...
    %computing population statistics and calculating the uncertainty based
    %on these statistics
    if(t>1)
        ...
        if(~isnan(rewMeanBest) && ~isnan(rewStdBest) &&...
            ...
            ODPU = uncertaintyFormula(rewMeanBest,rewStdBest,k1,rewMeanRest,rewStdRest,k2);
        end
        if(ODPU >= 0.1)
            uncertain = 1;
        elseif(isnan(ODPU))
%             uncertain = NaN;
        ...
        %switching between the strategies based on the population
        %statistics
        if(mfInx == mrInx)
            converged = 1;
            settings.pIL = 0;
    ...
    for i=1:numOfAgents
        ...
        if(t==1) %at time t always use individaul learning 
            strategySelection = 3;
        end
